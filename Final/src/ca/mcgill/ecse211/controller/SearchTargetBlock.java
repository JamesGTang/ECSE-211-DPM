...
/**
 * This class provides methods that implements the search algorithm for finding
 * the target block
 * 
 * @author jamestang
 *
...
public class SearchTargetBlock {
	...
	public SearchTargetBlock(int tb, int SZ_UR_X, int SZ_UR_Y, int SZ_LL_X, int SZ_LL_Y) throws OdometerExceptions {
		...
		Robot.usMotor.setSpeed(150);
		Robot.usMotor.setAcceleration(1000);
		...
		xRange = (SZ_UR_X - SZ_LL_X) / 2 * Robot.TILE_SIZE + 10;
		yRange = (SZ_UR_Y - SZ_LL_Y) / 2 * Robot.TILE_SIZE + 10;
	...
	public void SearchTarget() throws InterruptedException {
		for (int i = 1; i <= 4; i++) {
			...
			if (direction == 1 && !isTargetBlockFound) {
				...
				Robot.centerRobot((SZ_UR_X + 0.5) * Robot.TILE_SIZE, (SZ_UR_Y + 0.5) * Robot.TILE_SIZE);
				// robot heading alone y
				isMovingX = false;
				System.out.println("Moving alone y, vertice:  " + direction);
	...
	/**
	 * Move close to the block so the light sensor can determine the color of the
	 * block
	 * 
	 * @param distance
	 *            distance from robot to the block upon detection
	 */
	public void discoverBlock(double distance) {
		...
		if (!ifFound) {
			...
			System.out.println("Turn to faceblock: " + angleToPerf);
			...
			System.out.println("Finding color");
	 ...
	 * 
	 * @param xPrev
	 *            the x coordinate of the search path
	 * @param yPrev
	 *            the y coordinate of the search path
	 * @param angleToPerf
	...
	public boolean verifyDistance(double distance) {
		/*
		 * double totalDistance = 0; boolean isDistanceValid = false;
		 * 
		 * for (int i = 0; i < 5; i++) { totalDistance += Robot.getDistance();
		 * System.out.println("average distance: " + totalDistance); }
		 * 
		 * if (Math.abs(totalDistance / 5 - distance) < 5) { isDistanceValid = true; }
		 * System.out.println("Detected distance: " + isDistanceValid);
		 * System.out.println("Verify distance: " + isDistanceValid); return
		 * isDistanceValid;
		 */
		return true;
	}
	 ...
	 * 
	 * @param double
	 *            distance, the distance detected by ultrasonic sensor
	 */
	 ...
	 * 
	 * @param lightval
	 *            the array of values of R G B
	 * @return true if it is red
	 */
	private boolean isRed(double lightval[]) {

		if (lightval[0] < redBlockMean[0] + 2 * redBlockDev[0] && lightval[0] > redBlockMean[0] - 2 * redBlockDev[0]
	...
	private boolean isBlue(double lightval[]) {
		if (lightval[0] < blueBlockMean[0] + 2 * blueBlockDev[0] && lightval[0] > blueBlockMean[0] - 2 * blueBlockDev[0]
